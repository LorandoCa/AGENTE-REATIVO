...
PROB_MUTATION = 0.008 #1.0/GENOTYPE_SIZE
STD_DEV = 0.1
...
def objective_function(observation_history):
    ...
    for i in range(len(observation_history)):
        obs = observation_history[i]
        x, y, vx, vy, theta, vtheta, l, r = obs
        theta_n = abs(theta) / np.pi

        dist = abs(x) + abs(y)
        vel = abs(vx) + abs(vy)
        ang = theta_n + abs(vtheta)

        fitness_base = - (2.0 * dist + 1.5 * vel + 1.2 * ang)

        # progresso
        if i > 0:
            prev = abs(observation_history[i-1][0]) + abs(observation_history[i-1][1])
            curr = abs(x) + abs(y)
            progress += (prev - curr)
    ...
    bonus_pernas = 0
    if ultima_obs[6] and ultima_obs[7]:
        bonus_pernas = 5
    elif ultima_obs[6] or ultima_obs[7]:
        bonus_pernas = 1

    sucesso = check_successful_landing(ultima_obs)
    bonus_sucesso = 200 if sucesso else -200

    fitness_total = fitness_base + progress + bonus_pernas + bonus_sucesso

    return fitness_total, sucesso    
"""
...
def objective_function(observation_history):
    ...
    fitness = (0.25*x_score + 0.25*vy_score + 0.15*vx_score + 0.15*theta_score + 0.10*vstab_score + 0.10*legs_score)
...
def tournament_selection(population, k=3): # Considerar afinar o parâmetro
...
def subtree_swap_crossover(p1, p2):
    g1 = p1['genotype']
    g2 = p2['genotype']
    
    # escolhe um "bloco" (simulando uma camada)
    cut1 = random.randint(0, GENOTYPE_SIZE - 1)
    cut2 = random.randint(cut1, GENOTYPE_SIZE)
    
    child = g1[:cut1] + g2[cut1:cut2] + g1[cut2:]
